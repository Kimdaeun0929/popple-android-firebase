...
## 👥 팀 구성 및 역할
### 👩‍💻 김다은 (PM + 백엔드, [@kimdaeun0929](https://github.com/kimdaeun0929))
- 팀장 / PM
- 프로젝트 일정 관리 및 역할 분담
...
### 👩‍💻 김유정 (프론트엔드, [@Hello11234567](https://github.com/Hello11234567))
- 설계된 UI 디자인을 기반으로 XML 레이아웃 구현
- 사용자에게 보여지는 화면 구성 및 스타일링
- 실시간 채팅 기능 구현
  - 채팅 메시지를 DB에 저장
  - 메시지 변경 사항을 어플리케이션 화면에 실시간 반영
 
### 👥 기타 팀원
- UI/UX 디자인 및 초기 목업 제작
- 로그인 및 회원가입 기능 구현
- 지도 API를 활용한 위치 기반 지도 기능 구현
- DB 데이터를 불러와 화면에 표시하는 정보 페이지 구현
...
## 🧠 기술적 고민 및 해결 (김유정 담당 기준)
🔹실시간 채팅 메시지 반영
- Firebase Realtime Database를 활용한 채팅 메시지 데이터 관리
- 데이터 변경 감지를 통한 채팅 메시지 실시간 화면 반영

🔹채팅 메시지 UI 구현
- RecyclerView와 ChatAdapter를 활용한 채팅 메시지 리스트 구성
- 사용자 닉네임 비교를 통한 좌/우 채팅 버블 UI 구현

🔹채팅방 목록 데이터 관리
- ArrayList 기반 채팅방 목록 데이터 관리
- SharedPreferences를 활용한 채팅방 목록 로컬 저장 및 유지
